...
def get_linear_schedule_with_warmup_when_freeze(optimizer, num_warmup_steps, num_training_steps,num_freeze_steps, last_epoch=-1):
    """ Create a schedule with a learning rate that decreases linearly after
    linearly increasing during a warmup period.
    """
    def lr_lambda(current_step):
        if current_step < num_freeze_steps:return 1
            # return max(0.0, float(num_freeze_steps - current_step) / float(max(1, num_freeze_steps)))
        current_step -= num_freeze_steps
        if current_step < num_warmup_steps:
