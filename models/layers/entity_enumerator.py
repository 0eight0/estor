...
class Estor(nn.Module):
    def __init__(self, config):
        ...
        self.out_self_attention = nn.MultiheadAttention(self.tag_hidden_size, num_heads=config.num_attention_heads,
                                                    dropout=config.attention_probs_dropout_prob, batch_first=True)
        # self.out_self_attention = nn.MultiheadAttention(embed_dim=hidden_size, num_heads=config.num_attention_heads, batch_first=True)
        # old encoder                                            
        ...
        # 流入LSTM层
        self.in_lstm = nn.LSTM(hidden_size, hidden_size // 2, num_layers=1, bidirectional=True, batch_first=True)
    ...
    def get_result(self,X,tag_id):
        ...
        if self.if_add_a_self_attention:# old sel_att
            new_X, _ = self.self_attention(X, X, X)
            X = new_X+X


            # input_stream='self_attention'
            # assert input_stream in ['input_lstm','self_attention']
            # if input_stream == 'self_attention':
            #     new_X, _ = self.self_attention(X, X, X)
            #     X = new_X+X
            # else:
            #     # print(X.shape)
            #     new_X, _ = self.in_lstm(X)
            #     X = new_X+X



        # if self.if_add_a_self_attention:# new concat
    ...
    def forward(self, word_embedding,attention_mask, tag_to_spans):  # tag_to_span是一个加了batch的dict
        ...
        if banben == 'lite':
            ...
            # 转换attention_mask以适用于MultiheadAttention，设置填充位置为负无穷
            mask = attention_mask == 0
            output,_=self.out_self_attention(output_embedding,output_embedding,output_embedding,key_padding_mask=mask)

            # 应用 MultiheadAttention 注意力机制
            # 注意，我们需要将扩展的attention_mask传给forward方法的key_padding_mask参数
            # output,_= self.out_self_attention(output_embedding, output_embedding,output_embedding, key_padding_mask=attention_mask.bool())
            # output = output+output_embedding
