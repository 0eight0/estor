...
class Estor(nn.Module):
    def __init__(self, config):
        ...
        # if self.if_contrastive_learn:self._init_tags()
        ...
        self.reshape_linear = nn.Linear(hidden_size,int(hidden_size*self.tagging_rate))
        ...
        if self.if_merge_by_add:
            ...
            self.feed_forward = nn.Sequential(
                nn.Linear(int(hidden_size+hidden_size*self.tagging_rate), config.intermediate_size),
                nn.ReLU(),
                nn.Linear(config.intermediate_size, hidden_size))
    ...
    def get_result(self,X,tag_id):
        ...
        # print(X.shape,tag_embedding.shape)
